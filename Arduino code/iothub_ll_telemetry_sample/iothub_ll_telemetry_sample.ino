...
unsigned long startyMillis;
unsigned long startDHTMillis;
unsigned long currentyMillis;
const unsigned long period = 5*1000; //5 seconds
...
void loop(void) {
  currentyMillis = millis();
  digitalSoil = digitalRead(SOILSENSORD);
  analogSoil = analogRead(SOILSENSORA);
  if (currentyMillis - startDHTMillis >= dht.getMinimumSamplingPeriod()) {
    temp = dht.getTemperature();
    hum = dht.getHumidity();
    startDHTMillis = currentyMillis;
  }
  sprintf(telemetry_msg, "{\"test_message\": \"test\",\"analogSoil\": %d,\"temp\": %d,\"hum\": %d}", analogSoil, temp, hum);

  if (currentyMillis - startyMillis >= period) {
    startyMillis = currentyMillis;  //new timer
